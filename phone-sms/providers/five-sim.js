...
(function attachFiveSimSmsProvider(root, factory) {
  ...
  const DEFAULT_MAX_USES = Number.MAX_SAFE_INTEGER;
  ...
  function normalizeFiveSimProduct(value = '') {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '') || DEFAULT_PRODUCT;
  }
  ...
  function resolveProduct(state = {}, fallback = DEFAULT_PRODUCT) {
    return normalizeFiveSimProduct(
      state?.fiveSimProduct
      || state?.product
      || fallback
    );
  }
  ...
  function normalizeActivation(record, fallback = {}) {
    ...
    return {
      ...
      serviceCode: normalizeFiveSimProduct(record.serviceCode || record.product || fallback.serviceCode || DEFAULT_PRODUCT),
      countryId,
      countryLabel,
  ...
  async function buyActivationWithPrice(state = {}, countryConfig, maxPrice, deps = {}) {
    ...
    const payload = await fetchJson(
      config,
      `/v1/user/buy/activation/${encodeURIComponent(normalizeFiveSimCountryId(countryConfig.id))}/${encodeURIComponent(operator)}/${encodeURIComponent(product)}`,
      {
        query,
  ...
  async function reuseActivation(state = {}, activation, deps = {}) {
    ...
    const reuseProduct = normalizeFiveSimProduct(
      normalizedActivation.serviceCode
      || state?.fiveSimProduct
      || DEFAULT_PRODUCT
    );
    const payload = await fetchJson(config, `/v1/user/reuse/${encodeURIComponent(reuseProduct)}/${numberWithoutPlus || phoneDigits}`, {
      actionLabel: '5sim 复用手机号',
    });
    return normalizeActivation(payload, { ...normalizedActivation, serviceCode: reuseProduct });
  }
