...
- Cloudflare Temp Email
  - 标准协议下，Step 4 / Step 8 仍优先走 `/admin/mails` 与 `x-admin-auth`。
  - 当 `Temp API` 指向当前已适配的 `mailfree.nanzheyin.workers.dev` 时，验证码轮询会切到 `mailfree` 兼容分支：后台复用浏览器现有登录态，带 `credentials: include` 请求 `/api/emails?mailbox=<目标邮箱>`，再把返回的 `sender / subject / content / html_content / received_at` 归一化后复用现有验证码提取与时间窗过滤逻辑。
  - 这条兼容链路当前只接管“收码”；邮箱生成仍沿用原有 Cloudflare / Cloudflare Temp Email 配置与生成逻辑。
- Cloud Mail
- 自定义邮箱池
- iCloud 隐私邮箱
...
- `163`、`163 VIP`、`126` 和 `QQ` 已从邮箱服务下拉与 `manifest.json` 注入入口移除。
- 旧配置中的 `163 / 163-vip / 126 / qq` 会按 `normalizeMailProvider()` 回退到 `luckmail-api`。
- `content/mail-163.js` 和 `content/qq-mail.js` 仅作为历史脚本保留，不再作为当前邮箱服务入口。
...
- `icloudTargetMailboxType = forward-mailbox` 时，注册邮箱仍由 iCloud Hide My Email 生成，但 Step 4 / Step 8 改为打开 `icloudForwardMailProvider` 指定的转发目标邮箱收码；当前只支持 `gmail`。
