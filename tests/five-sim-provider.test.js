...
test('5sim provider maps countries and prices', async () => {
  ...
  assert.deepStrictEqual(provider.supportedCountries, []);
});
...
test('5sim provider buys, checks, finishes, cancels, bans, and reuses activation', async () => {
  ...
  assert.equal(activation.maxUses, Number.MAX_SAFE_INTEGER);
...
test('5sim provider uses configured product for price lookup, activation, and reuse', async () => {
  const requests = [];
  const provider = api.createProvider({
    fetchImpl: async (url, options = {}) => {
      const parsed = new URL(url);
      requests.push({ url: parsed, options });
      if (parsed.pathname === '/v1/guest/products/vietnam/any') {
        return createTextResponse({ telegram: { Category: 'activation', Qty: 4, Price: 7.5 } });
      }
      if (parsed.pathname === '/v1/guest/prices') {
        return createTextResponse({ vietnam: { any: { telegram: { cost: 7.5, count: 4 } } } });
      }
      if (parsed.pathname === '/v1/user/buy/activation/vietnam/any/telegram') {
        return createTextResponse({ id: 3001, phone: '+84901111111', country: 'vietnam', product: 'telegram', operator: 'any', status: 'PENDING' });
      }
      if (parsed.pathname === '/v1/user/reuse/telegram/84901111111') {
        return createTextResponse({ id: 3002, phone: '+84901111111', country: 'vietnam', product: 'telegram', status: 'PENDING' });
      }
      throw new Error(`unexpected ${parsed.pathname}`);
    },
    sleepWithStop: async () => {},
  ...
  assert.equal(activation.serviceCode, 'telegram');
  assert.equal(reused.serviceCode, 'telegram');
  assert.deepStrictEqual(
    requests.map((entry) => entry.url.pathname),
    [
      '/v1/guest/products/vietnam/any',
      ...
      '/v1/user/reuse/telegram/84901111111',
    ]
  );
  assert.equal(requests[1].url.searchParams.get('product'), 'telegram');
});
