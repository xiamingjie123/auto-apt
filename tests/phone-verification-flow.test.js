...
test('phone verification helper does not append reuse flag on new 5sim activation purchases', async () => {
  const requests = [];
  const helpers = api.createPhoneVerificationHelpers({
    addLog: async () => {},
    ensureStep8SignupPageReady: async () => {},
  ...
  assert.equal(activation.phoneNumber, '+84901112233');
  const buy = requests.find((entry) => entry.pathname === '/v1/user/buy/activation/vietnam/any/openai');
  assert.ok(buy);
  assert.equal(buy.search.get('reuse'), null);
});
...
test('phone verification helper skips page resend for 5sim timeouts and rotates number directly', async () => {
  ...
  let currentState = {
    phoneSmsProvider: '5sim',
    fiveSimApiKey: 'five-token',
    fiveSimCountryId: 'indonesia',
    fiveSimCountryLabel: '印度尼西亚 (Indonesia)',
...
test('phone verification helper keeps polling the same 5sim activation when resend action is WhatsApp', async () => {
  const requests = [];
  const messages = [];
  const submittedNumbers = [];
  const statusCallsById = {};
  ...
  try {
    ...
    const helpers = api.createPhoneVerificationHelpers({
      ...
      fetchImpl: async (url, options = {}) => {
        ...
        if (parsedUrl.pathname === '/v1/guest/products/indonesia/any') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ whatsapp: { Category: 'activation', Qty: 3, Price: 9.5 } }),
          };
        }
        if (parsedUrl.pathname === '/v1/guest/prices') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ indonesia: { any: { whatsapp: { cost: 9.5, count: 3 } } } }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/buy/activation/indonesia/any/whatsapp') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 540001, phone: '+628111111111', country: 'indonesia', product: 'whatsapp', operator: 'any', status: 'PENDING' }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/check/540001') {
          statusCallsById['540001'] = (statusCallsById['540001'] || 0) + 1;
          if (statusCallsById['540001'] === 1) {
            return {
              ok: true,
              status: 200,
              text: async () => JSON.stringify({ id: 540001, phone: '+628111111111', status: 'PENDING', sms: [] }),
            };
          }
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 540001, phone: '+628111111111', status: 'RECEIVED', sms: [{ text: 'WhatsApp code 123456' }] }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/finish/540001') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ status: 'FINISHED' }),
    ...
    assert.deepStrictEqual(submittedNumbers, ['+628111111111']);
    assert.equal(messages.includes('RETURN_TO_ADD_PHONE'), false);
    assert.equal(statusCallsById['540001'], 2, '5sim WhatsApp flow should keep polling the same activation after the first timeout window');
    assert.deepStrictEqual(
      requests
        .filter((entry) => entry.url.pathname === '/v1/user/buy/activation/indonesia/any/whatsapp')
        .map((entry) => entry.url.pathname),
      ['/v1/user/buy/activation/indonesia/any/whatsapp']
    );
  } finally {
    Date.now = realDateNow;
  }
...
test('phone verification helper re-acquires a 5sim whatsapp product number and keeps the previous number in memory', async () => {
  const requests = [];
  const submittedNumbers = [];
  const statusCallsById = {};
  let currentState = {
    ...
    currentPhoneActivation: null,
    reusablePhoneActivation: {
      activationId: 'old-openai-activation',
  ...
  try {
    ...
    const helpers = api.createPhoneVerificationHelpers({
      ...
      fetchImpl: async (url, options = {}) => {
        ...
        requests.push({ url: parsedUrl, options });
        if (parsedUrl.pathname === '/v1/guest/products/indonesia/any') {
          if (parsedUrl.searchParams.get('product') === 'whatsapp') {
            throw new Error('guest products should not use query product');
          }
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({
              openai: { Category: 'activation', Qty: 3, Price: 9.5 },
              whatsapp: { Category: 'activation', Qty: 2, Price: 8.1 },
            }),
          };
        }
        if (parsedUrl.pathname === '/v1/guest/prices') {
          const product = parsedUrl.searchParams.get('product');
          if (product === 'openai') {
            return {
              ok: true,
              status: 200,
              text: async () => JSON.stringify({ indonesia: { any: { openai: { cost: 9.5, count: 3 } } } }),
            };
          }
          if (product === 'whatsapp') {
            return {
              ok: true,
              status: 200,
              text: async () => JSON.stringify({ indonesia: { any: { whatsapp: { cost: 8.1, count: 2 } } } }),
            };
          }
        }
        if (parsedUrl.pathname === '/v1/user/buy/activation/indonesia/any/openai') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 550001, phone: '+628111111111', country: 'indonesia', product: 'openai', operator: 'any', status: 'PENDING' }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/buy/activation/indonesia/any/whatsapp') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 550002, phone: '+628222222222', country: 'indonesia', product: 'whatsapp', operator: 'any', status: 'PENDING' }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/check/550001') {
          statusCallsById['550001'] = (statusCallsById['550001'] || 0) + 1;
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 550001, phone: '+628111111111', status: 'PENDING', sms: [] }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/check/550002') {
          statusCallsById['550002'] = (statusCallsById['550002'] || 0) + 1;
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ id: 550002, phone: '+628222222222', status: 'RECEIVED', sms: [{ text: 'WhatsApp code 999888' }] }),
          };
        }
        if (parsedUrl.pathname === '/v1/user/cancel/550001') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ status: 'CANCELED' }),
        ...
        if (parsedUrl.pathname === '/v1/user/finish/550002') {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ status: 'FINISHED' }),
        ...
        throw new Error(`Unexpected 5sim path: ${parsedUrl.pathname} ? ${parsedUrl.search}`);
      },
      getOAuthFlowStepTimeoutMs: async (defaultTimeoutMs) => defaultTimeoutMs,
      getState: async () => ({ ...currentState }),
      sendToContentScriptResilient: async (_source, message) => {
    ...
    assert.deepStrictEqual(submittedNumbers, ['+628111111111', '+628222222222']);
    assert.equal(statusCallsById['550001'], 1);
    assert.equal(statusCallsById['550002'], 1);
    assert.deepStrictEqual(
      requests
        .filter((entry) => entry.url.pathname.includes('/buy/activation'))
        .map((entry) => entry.url.pathname),
      [
        '/v1/user/buy/activation/indonesia/any/openai',
        '/v1/user/buy/activation/indonesia/any/whatsapp',
      ]
    );
    assert.equal(currentState.reusablePhoneActivation.serviceCode, 'whatsapp');
    assert.equal(
      Array.isArray(currentState.phoneReusableActivationPool)
        && currentState.phoneReusableActivationPool.some((entry) => entry.activationId === 'old-openai-activation' && entry.serviceCode === 'openai'),
      true,
      'old openai 5sim number should stay in reusable memory pool after switching to a temporary whatsapp number'
    );
  } finally {
    Date.now = realDateNow;
  }
...
test('phone verification helper rotates Clash Verge proxy before acquiring a fresh phone number', async () => {
  let currentState = {
    phoneSmsProvider: '5sim',
    fiveSimApiKey: 'five-token',
    fiveSimCountryId: 'indonesia',
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    fetchImpl: async (url) => {
      const parsedUrl = new URL(url);
      if (parsedUrl.pathname === '/v1/guest/products/indonesia/any') {
        return { ok: true, status: 200, text: async () => JSON.stringify({ openai: { Category: 'activation', Qty: 1, Price: 9.5 } }) };
      }
      if (parsedUrl.pathname === '/v1/guest/prices') {
        return { ok: true, status: 200, text: async () => JSON.stringify({ indonesia: { any: { openai: { cost: 9.5, count: 1 } } } }) };
      }
      if (parsedUrl.pathname === '/v1/user/buy/activation/indonesia/any/openai') {
        return { ok: true, status: 200, text: async () => JSON.stringify({ id: 560001, phone: '+628333333333', country: 'indonesia', product: 'openai', status: 'PENDING' }) };
      }
      throw new Error(`Unexpected 5sim path: ${parsedUrl.pathname}`);
    },
    getState: async () => ({ ...currentState }),
    ...
    switchIpProxy: async (direction, options = {}) => {
      ...
      return { display: 'ChatGPT -> 🇺🇸 美国-03', current: { nodeName: '🇺🇸 美国-03' } };
    },
    sleepWithStop: async () => {},
    throwIfStopped: () => {},
  });
  ...
  const activation = await helpers.prepareSignupPhoneActivation(currentState);
  assert.equal(activation.phoneNumber, '+628333333333');
...
test('phone verification helper routes 5sim buy, check, and finish by current activation provider', async () => {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (parsedUrl.pathname === '/v1/guest/products/vietnam/any') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ telegram: { Category: 'activation', Qty: 3, Price: 9.5 } }),
        };
      }
      if (parsedUrl.pathname === '/v1/guest/prices') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ vietnam: { any: { telegram: { cost: 9.5, count: 3 } } } }),
        };
      }
      if (parsedUrl.pathname === '/v1/user/buy/activation/vietnam/any/telegram') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ id: 5001, phone: '+84901122334', country: 'vietnam', product: 'telegram', operator: 'any', status: 'PENDING' }),
        };
      }
      if (parsedUrl.pathname === '/v1/user/check/5001') {
        return {
  ...
  assert.equal(currentState.reusablePhoneActivation.serviceCode, 'telegram');
  ...
  assert.equal(buy.url.searchParams.get('reuse'), null);
  ...
  assert.deepStrictEqual(
    ...
    [
      ...
      '/v1/guest/prices',
      '/v1/user/buy/activation/vietnam/any/telegram',
      '/v1/user/check/5001',
      '/v1/user/finish/5001',
    ]
  );
...
test('phone verification helper routes 5sim reusable activation through reuse endpoint', async () => {
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    fetchImpl: async (url) => {
      const parsedUrl = new URL(url);
      requests.push(parsedUrl);
      if (parsedUrl.pathname === '/v1/user/reuse/telegram/84901122334') {
        return { ok: true, status: 200, text: async () => JSON.stringify({ id: 4002, phone: '+84901122334', country: 'vietnam', product: 'telegram', status: 'PENDING' }) };
      }
  ...
  assert.deepStrictEqual(
    ...
    [
      '/v1/user/reuse/telegram/84901122334',
      '/v1/user/check/4002',
      '/v1/user/finish/4002',
    ]
  );
...
test('phone verification helper keeps 5sim reusable activation after success when fiveSim reuse stays enabled', async () => {
  const requests = [];
  let currentState = {
    phoneSmsProvider: '5sim',
    fiveSimApiKey: 'demo-key',
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (parsedUrl.pathname === '/v1/user/buy/activation/vietnam/any/telegram') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ id: 5101, phone: '+84901100000', country: 'vietnam', product: 'telegram', operator: 'any', status: 'PENDING' }),
        };
      }
      if (parsedUrl.pathname === '/v1/user/check/5101') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ id: 5101, phone: '+84901100000', status: 'RECEIVED', sms: [{ code: '654321' }] }),
        };
      }
      if (parsedUrl.pathname === '/v1/user/finish/5101') {
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ status: 'FINISHED' }),
  ...
  assert.equal(currentState.reusablePhoneActivation.activationId, '5101');
  assert.equal(currentState.reusablePhoneActivation.phoneNumber, '+84901100000');
  assert.equal(currentState.reusablePhoneActivation.provider, '5sim');
  assert.equal(currentState.reusablePhoneActivation.serviceCode, 'telegram');
  assert.equal(currentState.reusablePhoneActivation.successfulUses, 1);
  assert.equal(currentState.reusablePhoneActivation.maxUses, Number.MAX_SAFE_INTEGER);
});
