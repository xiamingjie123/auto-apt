...
const mailfreeProvider = {
  isMailfreeBaseUrl(baseUrl) {
    return String(baseUrl || '').includes('mailfree.nanzheyin.workers.dev');
  },
  async requestMailfreeMessages(_config, address) {
...
test('pollCloudflareTempEmailVerificationCode uses mailfree protocol when base url matches', async () => {
  const api = createProviderApi({
    baseUrl: 'https://mailfree.nanzheyin.workers.dev',
    messages: [{
      id: 'mail-4',
      address: 'u9b70nbq2s@sdgsdf.109286.xyz',
      receivedDateTime: '2026-05-14T10:20:00.000Z',
      subject: 'OpenAI verification code',
      from: { emailAddress: { address: 'OpenAI <noreply@tm.openai.com>' } },
      bodyPreview: 'Your verification code is 135790.',
    }],
  });

  const result = await api.pollCloudflareTempEmailVerificationCode(4, {
    email: 'u9b70nbq2s@sdgsdf.109286.xyz',
    mailProvider: 'cloudflare-temp-email',
    emailGenerator: 'cloudflare',
    cloudflareTempEmailReceiveMailbox: 'u9b70nbq2s@sdgsdf.109286.xyz',
  }, {
    targetEmail: 'u9b70nbq2s@sdgsdf.109286.xyz',
    maxAttempts: 1,
    intervalMs: 1,
  });
