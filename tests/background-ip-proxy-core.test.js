...
test('Clash Verge provider reads non-China candidates from selector and switches target node', async () => {
  const api = loadIpProxyCore();
  global.fetch = async () => ({
    ok: true,
    status: 200,
  ...
  const result = await api.listClashVergeProxyCandidates(state);
  assert.equal(result.selector, 'ChatGPT');
