...
test('sidepanel html exposes phone verification toggle and multi-provider SMS rows', () => {
  ...
  assert.match(html, /id="row-five-sim-reuse-enabled"/);
  assert.match(html, /id="input-five-sim-reuse-enabled"/);
  ...
  assert.match(html, /id="row-five-sim-runtime-memory"/);
  assert.match(html, /id="display-five-sim-current-activation"/);
  assert.match(html, /id="display-five-sim-reusable-activation"/);
  assert.match(html, /id="display-five-sim-memory-pool"/);
...
function getFiveSimCountryLabelByCode(code = '') {
  return String(code || '').trim().toLowerCase() === 'thailand' ? 'Thailand' : 'Vietnam';
}
...
`)(normalizeIcloudTargetMailboxType, normalizeIcloudForwardMailProvider);
  ...
  assert.equal(payload.fiveSimReuseEnabled, true);
...
`)();
  ...
  assert.equal(api.latestState.fiveSimProduct, 'telegram');
...
test('loadFiveSimCountries uses dynamic 5sim country list instead of the legacy fixed whitelist', async () => {
  const api = new Function(`
let latestState = { fiveSimCountryOrder: ['brazil'] };
const DEFAULT_FIVE_SIM_COUNTRY_ID = 'vietnam';
const DEFAULT_FIVE_SIM_COUNTRY_LABEL = '越南 (Vietnam)';
const selectFiveSimCountry = {
  options: [],
  innerHTML: '',
...
${extractFunction('normalizeFiveSimCountryId')}
${extractFunction('normalizeFiveSimCountryLabel')}
${extractFunction('getFiveSimCountryDisplayNameByIso')}
${extractFunction('buildFiveSimCountryDisplayLabel')}
${extractFunction('buildFiveSimCountrySearchText')}
${extractFunction('parseFiveSimCountriesPayload')}
${extractFunction('getFiveSimRuntimeCountryOptions')}
${extractFunction('fetchFiveSimCountryOptions')}
${extractFunction('getFiveSimCountryOptionLabel')}
...
function applyFiveSimCountrySelection(countries = []) {
  fiveSimCountrySelectionOrder = normalizeFiveSimCountryFallbackList(countries).map((entry) => entry.id);
}
async function fetch(url, options = {}) {
  const parsed = new URL(url);
  fetchCalls.push({ url: parsed, options });
  return {
    ...
    status: 200,
    json: async () => ({
      brazil: { text_en: 'Brazil', iso: { BR: 1 }, prefix: { '+55': 1 } },
      france: { text_en: 'France', iso: { FR: 1 }, prefix: { '+33': 1 } },
    }),
  };
}
${extractFunction('loadFiveSimCountries')}
return {
  selectFiveSimCountry,
  fiveSimCountrySearchTextByCode,
  get fiveSimCountryOptionCache() { return fiveSimCountryOptionCache; },
  get fiveSimCountrySelectionOrder() { return fiveSimCountrySelectionOrder; },
  get fetchCalls() { return fetchCalls; },
  loadFiveSimCountries,
};
`)();

  await api.loadFiveSimCountries();

  assert.deepStrictEqual(
    api.selectFiveSimCountry.options.map((option) => option.value),
    ['brazil', 'france']
  );
  assert.deepStrictEqual(api.fiveSimCountrySelectionOrder, ['brazil']);
  assert.equal(api.fiveSimCountrySearchTextByCode.has('brazil'), true);
  assert.equal(api.fiveSimCountryOptionCache.length, 2);
  assert.deepStrictEqual(
    api.fetchCalls.map((entry) => entry.url.pathname),
    ['/v1/guest/countries']
  );
...
test('loadFiveSimCountries falls back to cache and current selection instead of a fixed whitelist', async () => {
  const api = new Function(`
let latestState = {
  fiveSimCountryId: 'argentina',
  fiveSimCountryLabel: '阿根廷 (Argentina)',
  fiveSimCountryOrder: ['argentina'],
};
const DEFAULT_FIVE_SIM_COUNTRY_ID = 'vietnam';
const DEFAULT_FIVE_SIM_COUNTRY_LABEL = '越南 (Vietnam)';
const selectFiveSimCountry = {
  options: [{ value: 'argentina', textContent: '阿根廷 (Argentina)', selected: true }],
  innerHTML: '',
  appendChild(option) { this.options.push(option); },
};
const fiveSimCountrySearchTextByCode = new Map([['argentina', '阿根廷 (Argentina) argentina']]);
let fiveSimCountrySelectionOrder = ['argentina'];
let fiveSimCountryOptionCache = [
  { id: 'argentina', code: 'argentina', label: '阿根廷 (Argentina)', searchText: '阿根廷 Argentina AR +54' },
  { id: 'brazil', code: 'brazil', label: '巴西 (Brazil)', searchText: '巴西 Brazil BR +55' },
];
const toastCalls = [];
const document = {
  createElement(tag) {
...
`)();
  ...
  assert.deepStrictEqual(api.fiveSimCountrySelectionOrder, ['argentina']);
  assert.equal(api.toastCalls.length, 0);
});
