...
importScripts(
  ...
  'background/mailfree-provider.js',
...
const IP_PROXY_SERVICE_VALUES = ['clash-verge'];
const IP_PROXY_ENABLED_SERVICE_VALUES = ['clash-verge'];
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  clashVergeControllerUrl: '',
  clashVergeSecret: '',
  clashVergeSelector: '',
  clashVergeSelectedGroup: '',
  clashVergeExcludeChina: true,
  clashVergeCurrentNodeName: '',
  ipProxyAccountList: '',
...
function normalizePhoneVerificationReplacementLimit(value, fallback = DEFAULT_PHONE_VERIFICATION_REPLACEMENT_LIMIT) {
  ...
  if (Math.floor(numeric) <= PHONE_REPLACEMENT_LIMIT_UNLIMITED) {
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'clashVergeControllerUrl':
      return normalizeClashVergeControllerUrl(value || '');
    case 'clashVergeSecret':
      return normalizeClashVergeSecret(value || '');
    case 'clashVergeSelector':
      return normalizeClashVergeSelectorName(value || '');
    case 'clashVergeSelectedGroup':
      return normalizeClashVergeSelectorName(value || '');
    case 'clashVergeExcludeChina':
      return value !== false;
    case 'clashVergeCurrentNodeName':
      return normalizeClashVergeNodeName(value || '');
...
async function listCloudflareTempEmailMessages(state, options = {}) {
  ...
  if (mailfreeProvider?.isMailfreeBaseUrl?.(config.baseUrl)) {
    const messages = await mailfreeProvider.requestMailfreeMessages(config, address, {
...
const mailfreeProvider = self.MultiPageBackgroundMailfreeProvider?.createMailfreeProvider({
  fetchImpl: fetch,
  normalizeCloudflareTempEmailAddress,
});
