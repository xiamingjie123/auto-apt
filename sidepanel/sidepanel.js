...
const rowClashVergeController = document.getElementById('row-clash-verge-controller');
const inputClashVergeController = document.getElementById('input-clash-verge-controller');
const btnDetectClashVergeController = document.getElementById('btn-detect-clash-verge-controller');
const rowClashVergeSecret = document.getElementById('row-clash-verge-secret');
const inputClashVergeSecret = document.getElementById('input-clash-verge-secret');
const btnToggleClashVergeSecret = document.getElementById('btn-toggle-clash-verge-secret');
const rowClashVergeSelector = document.getElementById('row-clash-verge-selector');
const inputClashVergeSelector = document.getElementById('input-clash-verge-selector');
const rowClashVergeExcludeChina = document.getElementById('row-clash-verge-exclude-china');
const inputClashVergeExcludeChina = document.getElementById('input-clash-verge-exclude-china');
...
const inputFiveSimReuseEnabled = document.getElementById('input-five-sim-reuse-enabled');
...
const rowFiveSimRuntimeMemory = document.getElementById('row-five-sim-runtime-memory');
const displayFiveSimCurrentActivation = document.getElementById('display-five-sim-current-activation');
const displayFiveSimReusableActivation = document.getElementById('display-five-sim-reusable-activation');
const displayFiveSimMemoryPool = document.getElementById('display-five-sim-memory-pool');
...
function collectSettingsPayload() {
    ...
    : ((rawValue = {}, fallbackState = {}) => {
      ...
      services.forEach((service) => {
        ...
        result[service] = {
          ...
          clashVergeControllerUrl: String(source.clashVergeControllerUrl || fallbackProfile.clashVergeControllerUrl || '').trim(),
          clashVergeSecret: String(source.clashVergeSecret || fallbackProfile.clashVergeSecret || ''),
          clashVergeSelector: String(source.clashVergeSelector || fallbackProfile.clashVergeSelector || '').trim(),
          clashVergeSelectedGroup: String(source.clashVergeSelectedGroup || fallbackProfile.clashVergeSelectedGroup || '').trim(),
          clashVergeExcludeChina: source.clashVergeExcludeChina !== false && fallbackProfile.clashVergeExcludeChina !== false,
  ...
  const ipProxyApiUrlRawValue = typeof inputIpProxyApiUrl !== 'undefined'
    ...
    : '';
  const clashVergeControllerRawValue = typeof inputClashVergeController !== 'undefined'
    ? inputClashVergeController?.value
    : '';
  const clashVergeSecretRawValue = typeof inputClashVergeSecret !== 'undefined'
    ? inputClashVergeSecret?.value
    : '';
  const clashVergeSelectorRawValue = typeof inputClashVergeSelector !== 'undefined'
    ? inputClashVergeSelector?.value
    : '';
  const clashVergeExcludeChinaRawValue = typeof inputClashVergeExcludeChina !== 'undefined'
    ? Boolean(inputClashVergeExcludeChina?.checked)
    : (latestState?.clashVergeExcludeChina !== false);
  ...
  const currentIpProxyServiceProfile = {
    ...
    clashVergeControllerUrl: String(clashVergeControllerRawValue || latestState?.clashVergeControllerUrl || '').trim(),
    clashVergeSecret: String(clashVergeSecretRawValue || latestState?.clashVergeSecret || ''),
    clashVergeSelector: String(clashVergeSelectorRawValue || latestState?.clashVergeSelector || '').trim(),
    clashVergeSelectedGroup: String(clashVergeSelectorRawValue || latestState?.clashVergeSelectedGroup || latestState?.clashVergeSelector || '').trim(),
    clashVergeExcludeChina: Boolean(clashVergeExcludeChinaRawValue),
  ...
  const ipProxyServiceProfiles = normalizeIpProxyServiceProfilesSafe({
    ...
    clashVergeControllerUrl: currentIpProxyServiceProfile.clashVergeControllerUrl,
    clashVergeSecret: currentIpProxyServiceProfile.clashVergeSecret,
    clashVergeSelector: currentIpProxyServiceProfile.clashVergeSelector,
    clashVergeSelectedGroup: currentIpProxyServiceProfile.clashVergeSelectedGroup,
    clashVergeExcludeChina: currentIpProxyServiceProfile.clashVergeExcludeChina,
  ...
  const fiveSimReuseEnabledValue = typeof inputFiveSimReuseEnabled !== 'undefined' && inputFiveSimReuseEnabled
    ? Boolean(inputFiveSimReuseEnabled.checked)
    : (latestState?.fiveSimReuseEnabled !== false);
  ...
  const resolvedFiveSimPrimaryCountryLabel = (typeof getFiveSimCountryLabelByCode === 'function'
    ? getFiveSimCountryLabelByCode(resolvedFiveSimPrimaryCountryId)
    : '')
    || normalizeFiveSimCountryLabel(
      resolvedFiveSimPrimaryCountryId === String(fiveSimCountry.id || '').trim().toLowerCase()
        ? fiveSimCountry.label
        : '',
  ...
  return {
    ...
    fiveSimReuseEnabled: fiveSimReuseEnabledValue,
    ...
    fiveSimCountryId: resolvedFiveSimPrimaryCountryId,
    fiveSimCountryLabel: resolvedFiveSimPrimaryCountryLabel,
...
function getFiveSimRuntimeCountryOptions() {
  const normalized = [];
  const seen = new Set();
  const append = (entry) => {
    ...
    seen.add(code);
    const label = normalizeFiveSimCountryLabel(
      entry?.label || formatFiveSimCountryDisplayLabel(code, entry?.text_en || entry?.eng || code, code),
      code
    );
    normalized.push({
      id: code,
      ...
      searchText: String(
        entry?.searchText
        || buildFiveSimCountrySearchText(entry || { code }, label, code)
      ).trim() || `${label} ${code}`,
    });
  };

  if (Array.isArray(fiveSimCountryOptionCache)) {
    fiveSimCountryOptionCache.forEach((entry) => append(entry));
  }

  Array.from(selectFiveSimCountry?.options || []).forEach((option) => {
    const code = normalizeFiveSimCountryCode(option.value, '');
    append({
      id: code,
      code,
      label: String(option.textContent || '').trim() || code,
      searchText: fiveSimCountrySearchTextByCode.get(code) || `${option.textContent || ''} ${code}`,
    });
  });

  normalizeFiveSimCountryFallbackList([
    ...(Array.isArray(latestState?.fiveSimCountryOrder) ? latestState.fiveSimCountryOrder : []),
    latestState?.fiveSimCountryId ? { id: latestState.fiveSimCountryId, label: latestState?.fiveSimCountryLabel } : null,
    ...
    append({
      ...
      code: entry.id,
      label: entry.label,
      searchText: `${entry.label} ${entry.id}`,
    });
  });

  append({
    id: DEFAULT_FIVE_SIM_COUNTRY_ID,
    code: DEFAULT_FIVE_SIM_COUNTRY_ID,
    label: DEFAULT_FIVE_SIM_COUNTRY_LABEL,
    searchText: `${DEFAULT_FIVE_SIM_COUNTRY_LABEL} ${DEFAULT_FIVE_SIM_COUNTRY_ID}`,
  });

  return normalized.sort((left, right) => String(left.label || '').localeCompare(String(right.label || '')));
}

async function fetchFiveSimCountryOptions() {
  ...
  try {
    ...
    fiveSimCountryOptionCache = items.map((entry) => ({ ...entry }));
    return items;
  } finally {
...
function getFiveSimCountryOptionLabel(code = '') {
  ...
  const fallback = getFiveSimRuntimeCountryOptions()
    .find((item) => normalizeFiveSimCountryCode(item.code || item.id, '') === normalizedCode);
  return fallback ? normalizeFiveSimCountryLabel(fallback.label, normalizedCode) : normalizedCode;
}
...
function renderFiveSimMemoryPanel(state = {}) {
  ...
  const shouldShow = provider === PHONE_SMS_PROVIDER_FIVE_SIM;
  if (rowFiveSimRuntimeMemory) {
    rowFiveSimRuntimeMemory.style.display = shouldShow ? '' : 'none';
  }
  if (!shouldShow) {
    return;
  }

  const currentActivation = normalizePhoneActivationState(
    state?.currentPhoneActivation || latestState?.currentPhoneActivation
  );
  const reusableActivation = normalizePhoneActivationState(
    state?.reusablePhoneActivation || latestState?.reusablePhoneActivation
  );
  ...
  if (displayFiveSimCurrentActivation) {
    displayFiveSimCurrentActivation.textContent = currentActivation
      ? buildPhoneActivationOptionLabel(currentActivation, '当前')
      : '未分配';
  }
  if (displayFiveSimReusableActivation) {
    displayFiveSimReusableActivation.textContent = reusableActivation
      ? buildPhoneActivationOptionLabel(reusableActivation, '复用')
      : '未保存';
  }
  if (displayFiveSimMemoryPool) {
    displayFiveSimMemoryPool.innerHTML = '';
    const normalizedEntries = reusablePool
      .map((entry) => normalizePhoneActivationState(entry))
      .filter((entry) => entry && entry.provider === PHONE_SMS_PROVIDER_FIVE_SIM);
    if (!normalizedEntries.length) {
      displayFiveSimMemoryPool.textContent = '暂无历史号码';
    } else {
      normalizedEntries.forEach((entry, index) => {
        const item = document.createElement('span');
        item.className = 'five-sim-memory-entry mono';
        item.textContent = `${index + 1}. ${buildPhoneActivationOptionLabel(entry, '池')}`;
        item.title = item.textContent;
        displayFiveSimMemoryPool.appendChild(item);
      });
    }
  }
}
...
function updateHeroSmsRuntimeDisplay(state = {}) {
  ...
  renderFiveSimMemoryPanel(state);
}
...
async function loadHeroSmsCountries() {
  ...
  if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    try {
      const optionItems = await fetchFiveSimCountryOptions();
      ...
      const fallbackItems = getFiveSimRuntimeCountryOptions().map((item) => ({
        id: item.id,
        label: item.label,
      ...
      if (typeof showToast === 'function') {
        showToast(`5sim 国家列表加载失败：${normalizeHeroSmsFetchErrorMessage(error)}（已切换为缓存/当前国家）`, 'warn', 2800);
      }
    }
  } else try {
    const controller = new AbortController();
...
async function loadFiveSimCountries() {
  ...
  const applyOptions = (items = []) => {
    ...
    if (Array.isArray(selectFiveSimCountry.options)) {
      selectFiveSimCountry.options.length = 0;
    }
  ...
  try {
    const items = await fetchFiveSimCountryOptions();
    applyOptions(items);
  } catch (error) {
    console.warn('Failed to load 5sim countries:', error);
    applyOptions(getFiveSimRuntimeCountryOptions());
  }
...
function applySettingsState(state) {
  ...
  const activeIpProxyProfile = typeof getIpProxyServiceProfile === 'function'
    ...
    : {
      ...
      clashVergeControllerUrl: String(state?.clashVergeControllerUrl || '').trim(),
      clashVergeSecret: String(state?.clashVergeSecret || ''),
      clashVergeSelector: String(state?.clashVergeSelector || '').trim(),
      clashVergeSelectedGroup: String(state?.clashVergeSelectedGroup || '').trim(),
      clashVergeExcludeChina: state?.clashVergeExcludeChina !== false,
  ...
  if (typeof inputClashVergeController !== 'undefined' && inputClashVergeController) {
    inputClashVergeController.value = String(activeIpProxyProfile.clashVergeControllerUrl || state?.clashVergeControllerUrl || '').trim();
  }
  if (typeof inputClashVergeSecret !== 'undefined' && inputClashVergeSecret) {
    inputClashVergeSecret.value = String(activeIpProxyProfile.clashVergeSecret || state?.clashVergeSecret || '');
  }
  if (typeof inputClashVergeSelector !== 'undefined' && inputClashVergeSelector) {
    inputClashVergeSelector.value = String(activeIpProxyProfile.clashVergeSelector || state?.clashVergeSelector || '').trim();
  }
  if (typeof inputClashVergeExcludeChina !== 'undefined' && inputClashVergeExcludeChina) {
    inputClashVergeExcludeChina.checked = activeIpProxyProfile.clashVergeExcludeChina !== false;
  }
  ...
  if (typeof inputFiveSimReuseEnabled !== 'undefined' && inputFiveSimReuseEnabled) {
    inputFiveSimReuseEnabled.checked = state?.fiveSimReuseEnabled !== false;
  }
...
btnDetectClashVergeController?.addEventListener('click', async () => {
  try {
    const response = await chrome.runtime.sendMessage({
      type: 'DETECT_CLASH_VERGE_CONTROLLER',
      source: 'sidepanel',
      payload: {
        controllerUrl: inputClashVergeController?.value || '',
      },
    });
    if (response?.error) {
      throw new Error(response.error);
    }
    if (inputClashVergeController && response?.controllerUrl) {
      inputClashVergeController.value = String(response.controllerUrl || '').trim();
    }
    if (inputClashVergeSelector && Array.isArray(response?.groups) && response.groups.length) {
      const preferred = response.groups.find((name) => /chatgpt/i.test(String(name || '').trim())) || response.groups[0];
      inputClashVergeSelector.value = String(preferred || '').trim();
    }
    showToast(
      `已探测到控制器：${String(response?.controllerUrl || '').trim() || 'unknown'}${Array.isArray(response?.groups) && response.groups.length ? `，策略组 ${response.groups.join(', ')}` : ''}`,
      'success',
      2600
    );
  } catch (err) {
...
async function switchPhoneSmsProvider(nextProvider) {
  ...
  if (previousProvider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    ...
    patch.fiveSimProduct = normalizeFiveSimProductValue(inputFiveSimProduct?.value || latestState?.fiveSimProduct);
  } else {
  ...
  if (inputFiveSimProduct) {
    inputFiveSimProduct.value = normalizeFiveSimProductValue(latestState?.fiveSimProduct);
  }
  ...
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (
        ...
        || message.payload.clashVergeControllerUrl !== undefined
        || message.payload.clashVergeSecret !== undefined
        || message.payload.clashVergeSelector !== undefined
        || message.payload.clashVergeSelectedGroup !== undefined
        || message.payload.clashVergeExcludeChina !== undefined
        ...
        if (message.payload.clashVergeControllerUrl !== undefined && inputClashVergeController) {
          inputClashVergeController.value = String(message.payload.clashVergeControllerUrl || '').trim();
        }
        if (message.payload.clashVergeSecret !== undefined && inputClashVergeSecret) {
          inputClashVergeSecret.value = String(message.payload.clashVergeSecret || '');
        }
        if (message.payload.clashVergeSelector !== undefined && inputClashVergeSelector) {
          inputClashVergeSelector.value = String(message.payload.clashVergeSelector || '').trim();
        }
        if (message.payload.clashVergeExcludeChina !== undefined && inputClashVergeExcludeChina) {
          inputClashVergeExcludeChina.checked = Boolean(message.payload.clashVergeExcludeChina);
        }
        if (hasIpProxyConfigPayload) {
          const activeProxyProfile = typeof getIpProxyServiceProfile === 'function'
            ...
            : {
              ...
              clashVergeControllerUrl: String(mergedProxyState?.clashVergeControllerUrl || '').trim(),
              clashVergeSecret: String(mergedProxyState?.clashVergeSecret || ''),
              clashVergeSelector: String(mergedProxyState?.clashVergeSelector || '').trim(),
              clashVergeSelectedGroup: String(mergedProxyState?.clashVergeSelectedGroup || '').trim(),
              clashVergeExcludeChina: mergedProxyState?.clashVergeExcludeChina !== false,
      ...
      if (message.payload.fiveSimReuseEnabled !== undefined && inputFiveSimReuseEnabled) {
        inputFiveSimReuseEnabled.checked = Boolean(message.payload.fiveSimReuseEnabled);
      }
