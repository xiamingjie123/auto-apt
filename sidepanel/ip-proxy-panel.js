...
function normalizeIpProxyServiceProfile(rawValue = {}) {
  ...
  return {
    ...
    clashVergeControllerUrl: String(raw.clashVergeControllerUrl || '').trim(),
    clashVergeSecret: String(raw.clashVergeSecret || ''),
    clashVergeSelector: String(raw.clashVergeSelector || '').trim(),
    clashVergeSelectedGroup: String(raw.clashVergeSelectedGroup || '').trim(),
...
function buildCurrentIpProxyServiceProfileFromInputs() {
  ...
  return normalizeIpProxyServiceProfile({
    ...
    clashVergeControllerUrl: latestState?.clashVergeControllerUrl || '',
    clashVergeSecret: latestState?.clashVergeSecret || '',
    clashVergeSelector: latestState?.clashVergeSelector || 'ChatGPT',
    clashVergeSelectedGroup: latestState?.clashVergeSelectedGroup || latestState?.clashVergeSelector || 'ChatGPT',
    clashVergeExcludeChina: latestState?.clashVergeExcludeChina !== false,
...
function buildIpProxyStatePatchFromServiceProfile(service = '', profile = {}) {
  ...
  return {
    ...
    clashVergeControllerUrl: normalizedProfile.clashVergeControllerUrl,
    clashVergeSecret: normalizedProfile.clashVergeSecret,
    clashVergeSelector: normalizedProfile.clashVergeSelector,
    clashVergeSelectedGroup: normalizedProfile.clashVergeSelectedGroup || normalizedProfile.clashVergeSelector,
    clashVergeExcludeChina: normalizedProfile.clashVergeExcludeChina !== false,
...
function updateIpProxyUI(state = latestState) {
  ...
  const isClashVerge = service === 'clash-verge';
  ...
  if (rowIpProxyApiUrl) {
    rowIpProxyApiUrl.style.display = showSettings && apiModeAvailable && isApiMode && !isClashVerge ? '' : 'none';
  }
  if (rowClashVergeController) {
    rowClashVergeController.style.display = showSettings && isClashVerge ? '' : 'none';
  }
  if (rowClashVergeSecret) {
    rowClashVergeSecret.style.display = showSettings && isClashVerge ? '' : 'none';
  }
  if (rowClashVergeSelector) {
    rowClashVergeSelector.style.display = showSettings && isClashVerge ? '' : 'none';
  }
  if (rowClashVergeExcludeChina) {
    rowClashVergeExcludeChina.style.display = showSettings && isClashVerge ? '' : 'none';
  }
  ...
  if (rowIpProxyHost) {
    rowIpProxyHost.style.display = showSettings && isAccountMode && !isClashVerge ? '' : 'none';
  }
  if (rowIpProxyPort) {
    rowIpProxyPort.style.display = showSettings && isAccountMode && !isClashVerge ? '' : 'none';
  }
  if (rowIpProxyProtocol) {
    rowIpProxyProtocol.style.display = showSettings && !isClashVerge ? '' : 'none';
  }
  if (rowIpProxyUsername) {
    rowIpProxyUsername.style.display = showSettings && isAccountMode && !isClashVerge ? '' : 'none';
  }
  if (rowIpProxyPassword) {
    rowIpProxyPassword.style.display = showSettings && isAccountMode && !isClashVerge ? '' : 'none';
  }
  if (rowIpProxyRegion) {
    rowIpProxyRegion.style.display = showSettings && isAccountMode && !isClashVerge ? '' : 'none';
  }
  ...
  if (selectIpProxyService) {
    ...
    selectIpProxyService.disabled = false;
  }
  ...
  if (ipProxyApiPanel) {
    ...
    ipProxyApiPanel.hidden = !apiModeAvailable && !isClashVerge;
    ipProxyApiPanel.style.display = showSettings && (apiModeAvailable || isClashVerge) ? '' : 'none';
  }
  if (inputIpProxyApiUrl) {
