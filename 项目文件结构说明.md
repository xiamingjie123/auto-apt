...
- `background/mailfree-provider.js`：Mailfree 协议兼容层，负责识别特定 `mailfree` 站点、复用浏览器登录态请求 `/api/emails?mailbox=...` 读取邮件列表，并在命中验证码后通过 `DELETE /api/email/:id` 删除单封邮件；返回结构会统一归一化为 Cloudflare Temp Email 验证码轮询链路可复用的邮件模型；不接管标准 `/admin/mails` / `/admin/new_address` 协议。
...
- `content/mail-163.js`：历史网易邮箱轮询脚本，当前不再从扩展清单或邮箱服务下拉入口启用。
...
- `content/qq-mail.js`：历史 QQ 邮箱轮询脚本，当前不再从扩展清单或邮箱服务下拉入口启用。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  码或从 Gmail 转发目标邮箱收码；当邮箱服务或邮箱生成器为 Cloud Mail 时，额外显示 API 地址、管理员账号、接收邮箱和生成域名配置行；来源下拉框当前支持 `CPA / SUB2API / Codex2API`，其中 Codex2API 额外提供后台地址和管理密
...
- `tests/cloudflare-temp-email-provider.test.js`：测试 Cloudflare Temp Email provider 的轮询与目标邮箱选择逻辑，并覆盖 `mailfree` 协议兼容分支。
- `tests/cloudflare-temp-email-utils.test.js`：测试 Cloudflare Temp Email 工具层的 URL、域名、邮件解析逻辑。
- `tests/mailfree-provider.test.js`：测试 `mailfree` 协议识别与邮件字段归一化逻辑。
