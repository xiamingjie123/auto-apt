...
- `background/mailfree-provider.js`：Mailfree 协议兼容层，仅负责识别特定 `mailfree` 站点、复用浏览器登录态请求 `/api/emails?mailbox=...`，并把返回的 `sender / subject / content / html_content / received_at` 归一化为 Cloudflare Temp Email 验证码轮询链路可复用的统一邮件结构；不接管标准 `/admin/mails` / `/admin/new_address` 协议。
