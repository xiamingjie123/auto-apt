...
function isClashVergeProvider(provider = '') {
  return normalizeIpProxyProviderValue(provider) === 'clash-verge';
}

function normalizeClashVergeControllerUrl(value = '') {
...
function getClashVergeControllerProbeCandidates(seed = '') {
  const normalizedSeed = normalizeClashVergeControllerUrl(seed);
...
function normalizeClashVergeSelectorName(value = '') {
  return String(value || '').trim();
}

function normalizeClashVergeSecret(value = '') {
  return String(value || '').trim();
}

function normalizeClashVergeNodeName(value = '') {
  return String(value || '').trim();
}

function buildClashVergeHeaders(state = {}) {
  const secret = normalizeClashVergeSecret(state?.clashVergeSecret);
  return {
...
function shouldExcludeChineseRegionForClashNode(name = '') {
  const text = String(name || '').trim();
  if (!text) {
    return true;
  }
  return /中国|台湾|香港|澳门|🇨🇳|🇭🇰|🇲🇴|tw\b|hk\b|cn\b/i.test(text);
}

function buildClashVergeNodeEntry(nodeName = '', state = {}) {
  const name = normalizeClashVergeNodeName(nodeName);
  if (!name) {
    return null;
  }
  return {
    ...
    nodeName: name,
    selector: normalizeClashVergeSelectorName(state?.clashVergeSelector || state?.clashVergeSelectedGroup || ''),
    controllerUrl: normalizeClashVergeControllerUrl(state?.clashVergeControllerUrl),
  };
}

async function fetchClashVergeJson(state = {}, path = '', options = {}) {
  const controllerUrl = normalizeClashVergeControllerUrl(state?.clashVergeControllerUrl);
  if (!controllerUrl) {
    throw new Error('Clash Verge External Controller 不能为空。');
  }
  const url = `${controllerUrl}${String(path || '').startsWith('/') ? '' : '/'}${String(path || '')}`;
  const timeoutMs = Number(options?.timeoutMs) > 0 ? Number(options.timeoutMs) : IP_PROXY_FETCH_TIMEOUT_MS;
  const response = await fetchWithTimeout(url, {
    method: options?.method || 'GET',
    cache: 'no-store',
    headers: {
      ...buildClashVergeHeaders(state),
  ...
  if (!response.ok) {
    throw new Error(`Clash Verge 控制器请求失败（HTTP ${response.status}）：${String(payload?.message || payload?.error || rawText || 'unknown error')}`.trim());
  }
  return payload;
}

async function fetchClashVergeProxyOverview(state = {}, options = {}) {
  return fetchClashVergeJson(state, '/proxies', options);
}

async function detectClashVergeController(state = {}, options = {}) {
  const candidates = getClashVergeControllerProbeCandidates(
    options?.controllerUrl || state?.clashVergeControllerUrl || ''
  );
  let lastError = null;
  for (const controllerUrl of candidates) {
    try {
      const payload = await fetchClashVergeProxyOverview({
        ...state,
        clashVergeControllerUrl: controllerUrl,
      }, options);
      ...
      const groups = Object.entries(proxies)
        ...
        .filter(Boolean);
      return { controllerUrl, groups };
    } catch (error) {
      lastError = error;
    }
  }
  throw lastError || new Error('未探测到可用的 Clash Verge External Controller。');
}

async function listClashVergeProxyCandidates(state = {}, options = {}) {
  const payload = await fetchClashVergeProxyOverview(state, options);
  const proxies = payload?.proxies && typeof payload.proxies === 'object' ? payload.proxies : {};
  const selector = normalizeClashVergeSelectorName(
    state?.clashVergeSelector
    || state?.clashVergeSelectedGroup
    || 'ChatGPT'
  );
  const selectorEntry = proxies?.[selector];
  const allCandidates = Array.isArray(selectorEntry?.all)
    ? selectorEntry.all.map((item) => String(item || '').trim()).filter(Boolean)
    : [];
  const candidates = allCandidates
    .filter((name) => !/^(DIRECT|REJECT|GLOBAL|PASS)$/i.test(name))
    .filter((name) => !Boolean(state?.clashVergeExcludeChina !== false && shouldExcludeChineseRegionForClashNode(name)))
    .map((name) => buildClashVergeNodeEntry(name, {
      ...state,
      clashVergeSelectedGroup: selector,
    }))
    .filter(Boolean);
  return {
    selector,
    now: String(selectorEntry?.now || '').trim(),
    candidates,
    raw: payload,
  };
}

async function switchClashVergeProxyNode(state = {}, entry = null, options = {}) {
  const targetEntry = entry && typeof entry === 'object' ? entry : getIpProxyCurrentEntryFromState(state);
  const selector = normalizeClashVergeSelectorName(
    options?.selector
    || targetEntry?.selector
    || state?.clashVergeSelector
    || state?.clashVergeSelectedGroup
    || 'ChatGPT'
  );
  const nodeName = normalizeClashVergeNodeName(options?.nodeName || targetEntry?.nodeName);
  if (!selector) {
    throw new Error('Clash Verge 策略组不能为空。');
  }
  if (!nodeName) {
    throw new Error('Clash Verge 目标节点不能为空。');
  }
  await fetchClashVergeJson(state, `/proxies/${encodeURIComponent(selector)}`, {
    method: 'PUT',
    body: { name: nodeName },
...
function normalizeIpProxyServiceProfile(rawValue = {}) {
  ...
  return {
    ...
    clashVergeControllerUrl: normalizeClashVergeControllerUrl(raw.clashVergeControllerUrl || ''),
    clashVergeSecret: normalizeClashVergeSecret(raw.clashVergeSecret || ''),
    clashVergeSelector: normalizeClashVergeSelectorName(raw.clashVergeSelector || ''),
    clashVergeSelectedGroup: normalizeClashVergeSelectorName(raw.clashVergeSelectedGroup || ''),
    clashVergeExcludeChina: raw.clashVergeExcludeChina !== false,
...
function buildIpProxyServiceProfileFromState(state = {}) {
  return normalizeIpProxyServiceProfile({
    ...
    clashVergeControllerUrl: state?.clashVergeControllerUrl,
    clashVergeSecret: state?.clashVergeSecret,
    clashVergeSelector: state?.clashVergeSelector,
    clashVergeSelectedGroup: state?.clashVergeSelectedGroup,
    clashVergeExcludeChina: state?.clashVergeExcludeChina,
...
async function pullIpProxyPoolFromApi(state = {}, options = {}) {
  ...
  if (isClashVergeProvider(provider)) {
    const result = await listClashVergeProxyCandidates(state, options);
    const maxItems = Math.max(1, Math.min(500, Number(options.maxItems) || 100));
    return result.candidates.slice(0, maxItems);
  }
  ...
  if (isClashVergeProvider(provider)) {
    ...
    if (!entry) {
      const status = {
        ...
        provider,
        error: 'Clash Verge 当前没有可切换的非中国区节点。',
        exitDetecting: false,
        exitIp: '',
        exitRegion: '',
        exitError: '',
    ...
    try {
      const switched = await switchClashVergeProxyNode(resolvedState, entry, options);
      const status = {
        enabled: true,
        applied: true,
        reason: 'applied',
        ...
        region: 'GLOBAL',
        username: '',
        entrySource: 'clash_verge_selector',
        hasAuth: false,
        provider,
        error: '',
        warning: '',
        ...
        exitError: '',
        exitSource: '',
        selector: switched.selector,
        nodeName: switched.nodeName,
      };
      await updateIpProxyRuntimeStatus(status);
      return status;
    } catch (error) {
      const status = {
        ...
        region: 'GLOBAL',
        hasAuth: false,
        error: error?.message || String(error || 'Clash Verge 切换失败'),
        exitDetecting: false,
        exitIp: '',
        exitRegion: '',
        exitError: '',
...
async function probeIpProxyExit(options = {}) {
  ...
  if (isClashVergeProvider(provider)) {
    const status = {
      ...
      applied: Boolean(state?.ipProxyEnabled),
      reason: Boolean(state?.ipProxyEnabled) ? 'applied' : 'disabled',
      provider,
      host: 'clash-verge',
      port: 9097,
      region: 'GLOBAL',
