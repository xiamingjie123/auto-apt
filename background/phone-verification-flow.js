(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    ...
    const DEFAULT_FIVE_SIM_MAX_USES = Number.MAX_SAFE_INTEGER;
    ...
    function canCurrentProviderReadWhatsAppChannel(state = {}, activation = null) {
      return getActivationProviderId(activation, state) === PHONE_SMS_PROVIDER_FIVE_SIM;
    }
    ...
    function isPhoneSmsReuseEnabled(state = {}) {
      const provider = normalizePhoneSmsProvider(state?.phoneSmsProvider);
      if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    ...
    function normalizeActivation(record) {
      ...
      const defaultMaxUses = provider === PHONE_SMS_PROVIDER_FIVE_SIM
        ? DEFAULT_FIVE_SIM_MAX_USES
      ...
      return {
        ...
        maxUses: Math.max(1, Math.floor(Number(record.maxUses) || defaultMaxUses)),
    ...
    function parseFiveSimActivationPayload(payload, fallback = {}) {
      ...
      return {
        ...
        maxUses: Math.max(1, Math.floor(Number(payload.maxUses ?? fallback.maxUses) || DEFAULT_FIVE_SIM_MAX_USES)),
        ...(() => {
    ...
    async function upsertReusableActivationPool(activation, options = {}) {
      ...
      const currentReusable = normalizeActivation(state?.[REUSABLE_PHONE_ACTIVATION_STATE_KEY]);
      const seededPool = [
        ...(currentReusable ? [currentReusable] : []),
        ...existingPool,
      ];
      const filtered = seededPool.filter((entry) => !isSameActivation(entry, normalized));
    ...
    async function acquirePhoneActivation(state = {}, options = {}) {
      ...
      const requestedFiveSimProduct = provider === PHONE_SMS_PROVIDER_5SIM
        ? normalizeFiveSimCountryCode(
          options?.fiveSimProductOverride || state?.fiveSimProduct,
          DEFAULT_FIVE_SIM_PRODUCT
        )
        : '';
      if (
      ...
      const scopedStateForProvider = (providerName) => ({
        ...
        ...(
          normalizePhoneSmsProvider(providerName) === PHONE_SMS_PROVIDER_5SIM && requestedFiveSimProduct
            ? { fiveSimProduct: requestedFiveSimProduct }
            : {}
        ),
      });
      const matchesRequestedFiveSimProduct = (candidateActivation) => {
        if (provider !== PHONE_SMS_PROVIDER_5SIM || !requestedFiveSimProduct) {
          return true;
        }
        ...
        return normalizeFiveSimCountryCode(
          normalizedCandidate.serviceCode,
          DEFAULT_FIVE_SIM_PRODUCT
        ) === requestedFiveSimProduct;
      };
      ...
      const canTryPreferredActivation = (
        ...
        && matchesRequestedFiveSimProduct(preferredActivation)
      ...
      if (canTryPreferredActivation) {
        try {
          ...
          if (preferredActivation.provider !== PHONE_SMS_PROVIDER_5SIM) {
            await removeReusableActivationFromPool(preferredActivation, { state }).catch(() => {});
          }
          await addLog(
            preferredActivation.provider === PHONE_SMS_PROVIDER_5SIM
              ? `步骤 9：手动选择的 5sim 号码 ${preferredActivation.phoneNumber} 当前不可复用，本次改为获取新号码，但保留历史记忆。${error.message}`
      ...
      if (reuseEnabled && (provider === PHONE_SMS_PROVIDER_HERO || provider === PHONE_SMS_PROVIDER_5SIM)) {
        for (const candidateActivation of reusableCandidates) {
          ...
          if (!matchesRequestedFiveSimProduct(candidateActivation)) {
            continue;
          }
          ...
          try {
            ...
            if (candidateActivation.provider === PHONE_SMS_PROVIDER_5SIM) {
              await addLog(
                `步骤 9：复用 5sim 历史号码 ${candidateActivation.phoneNumber} 失败，本次改为获取新号码，但保留该号码记忆。${error.message}`,
                'warn'
              );
            } else {
      ...
      const shouldRotateClashVergeBeforeNewNumber = Boolean(
        typeof switchIpProxy === 'function'
        && options?.rotateProxyBeforeAcquire !== false
        && Boolean(state?.ipProxyEnabled)
        && String(state?.ipProxyService || '').trim().toLowerCase() === 'clash-verge'
      );
      if (shouldRotateClashVergeBeforeNewNumber) {
        try {
          ...
          await addLog(
            `步骤 9：申请新号码前已切换 Clash Verge 代理：${String(switchResult?.display || switchResult?.current?.nodeName || '已切换到下一条').trim() || '已切换到下一条'}。`,
            'info'
          );
          state = await getState();
        } catch (error) {
          await addLog(`步骤 9：申请新号码前切换 Clash Verge 代理失败，将继续使用当前节点。${error.message}`, 'warn');
        }
      }
    ...
    async function markActivationReusableAfterSuccess(state, activation) {
      const normalizedActivation = normalizeActivation(activation);
      const provider = normalizedActivation?.provider || normalizePhoneSmsProvider(state?.phoneSmsProvider);
      if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
        if (state?.fiveSimReuseEnabled === false) {
          await clearReusableActivation();
          return;
        }
      } else if (!normalizeHeroSmsReuseEnabled(state?.heroSmsReuseEnabled)) {
        await clearReusableActivation();
        return;
      }
      if (!normalizedActivation) {
        ...
        try {
          ...
          if (windowIndex < timeoutWindows) {
            ...
            if (normalizedActivation.provider === PHONE_SMS_PROVIDER_5SIM) {
              try {
                ...
                if (isWhatsAppPhoneResendResult(resendProbeResult)) {
                  if (normalizeFiveSimCountryCode(normalizedActivation.serviceCode, DEFAULT_FIVE_SIM_PRODUCT) !== 'whatsapp') {
                    await addLog(
                      `步骤 9：页面重发入口显示 WhatsApp 通道，但当前 5sim 订单产品为 ${normalizedActivation.serviceCode || DEFAULT_FIVE_SIM_PRODUCT}，将重新申请 WhatsApp 号码。`,
                      'warn'
                    );
                    await clearPhoneRuntimeCountdown();
                    return {
                      code: '',
                      replaceNumber: true,
                      reason: 'whatsapp_requires_product_switch',
                      fiveSimProductOverride: 'whatsapp',
                    };
                  }
                  await addLog(
                    `步骤 9：页面重发入口显示 WhatsApp 通道（${resendProbeResult.channelText || resendProbeResult.text || 'WhatsApp'}），当前 5sim 订单已匹配 WhatsApp，继续轮询同一号码。`,
                    'warn'
                  );
                  continue;
                }
              } catch (probeError) {
                if (isStopRequestedError(probeError)) {
                  throw probeError;
                }
                await addLog(`步骤 9：检测 WhatsApp 重发通道失败，继续按 5sim 原逻辑轮询。${probeError.message}`, 'warn');
              }
            }
            if (!usePageResend) {
              await addLog(
            ...
            try {
              const resendProbeResult = await resendPhoneVerificationCode(tabId, { probeOnly: true });
              if (isWhatsAppPhoneResendResult(resendProbeResult)) {
                if (canCurrentProviderReadWhatsAppChannel(state, normalizedActivation)) {
                  if (normalizeFiveSimCountryCode(normalizedActivation.serviceCode, DEFAULT_FIVE_SIM_PRODUCT) !== 'whatsapp') {
                    await addLog(
                      `步骤 9：页面重发入口显示 WhatsApp 通道，但当前 5sim 订单产品为 ${normalizedActivation.serviceCode || DEFAULT_FIVE_SIM_PRODUCT}，将重新申请 WhatsApp 号码。`,
                      'warn'
                  ...
                  await addLog(
                    `步骤 9：页面重发入口显示 WhatsApp 通道（${resendProbeResult.channelText || resendProbeResult.text || 'WhatsApp'}），当前 5sim 订单将继续沿用并轮询接码平台消息。`,
                    'warn'
                  );
                  continue;
                }
              ...
              if (resendProbeResult?.probed) {
                ...
                if (isWhatsAppPhoneResendResult(resendResult)) {
                  if (canCurrentProviderReadWhatsAppChannel(state, normalizedActivation)) {
                    if (normalizeFiveSimCountryCode(normalizedActivation.serviceCode, DEFAULT_FIVE_SIM_PRODUCT) !== 'whatsapp') {
                      await addLog(
                        `步骤 9：页面重发入口切换为 WhatsApp 通道，但当前 5sim 订单产品为 ${normalizedActivation.serviceCode || DEFAULT_FIVE_SIM_PRODUCT}，将重新申请 WhatsApp 号码。`,
                        'warn'
                      );
                      await clearPhoneRuntimeCountdown();
                      return {
                    ...
                    await addLog(
                      `步骤 9：页面重发入口切换为 WhatsApp 通道（${resendResult.channelText || resendResult.text || 'WhatsApp'}），当前 5sim 订单将继续沿用并轮询接码平台消息。`,
                      'warn'
                    );
    ...
    async function completePhoneVerificationFlow(tabId, initialPageState = null, options = {}) {
      ...
      let temporaryFiveSimProductOverride = '';
        ...
        while (true) {
            ...
            if (!activation) {
              ...
              if (activation) {
                ...
                activation = await acquirePhoneActivation(state, {
                  ...
                  fiveSimProductOverride: temporaryFiveSimProductOverride,
                });
                shouldCancelActivation = true;
                await persistCurrentActivation(activation);
                temporaryFiveSimProductOverride = '';
              }
              addPhoneReentryWithSameActivation = 0;
          ...
          for (let attempt = 1; attempt <= DEFAULT_PHONE_SUBMIT_ATTEMPTS; attempt += 1) {
            ...
            if (codeResult.replaceNumber) {
              if (codeResult.fiveSimProductOverride) {
                temporaryFiveSimProductOverride = normalizeFiveSimCountryCode(
                  codeResult.fiveSimProductOverride,
                  DEFAULT_FIVE_SIM_PRODUCT
                );
              }
            ...
            if (submitResult.invalidCode) {
              ...
              if (remainingResendRequests > 0) {
                ...
                try {
                  const resendProbeResult = await resendPhoneVerificationCode(tabId, { probeOnly: true });
                  if (isWhatsAppPhoneResendResult(resendProbeResult)) {
                    if (canCurrentProviderReadWhatsAppChannel(state, activation)) {
                      if (normalizeFiveSimCountryCode(activation?.serviceCode, DEFAULT_FIVE_SIM_PRODUCT) !== 'whatsapp') {
                        temporaryFiveSimProductOverride = 'whatsapp';
                        shouldReplaceNumber = true;
                        replaceReason = 'whatsapp_requires_product_switch';
                        await addLog(
                          `步骤 9：验证码被拒后的重发入口显示 WhatsApp 通道，但当前 5sim 订单产品为 ${activation?.serviceCode || DEFAULT_FIVE_SIM_PRODUCT}，将重新申请 WhatsApp 号码。`,
                          'warn'
                        );
                        break;
                      }
                      await addLog(
                        `步骤 9：验证码被拒后的重发入口显示 WhatsApp 通道（${resendProbeResult.channelText || resendProbeResult.text || 'WhatsApp'}），当前 5sim 订单将继续沿用并轮询接码平台消息。`,
                        'warn'
                      );
                      continue;
                    }
                    ...
                    if (isWhatsAppPhoneResendResult(resendResult)) {
                      if (canCurrentProviderReadWhatsAppChannel(state, activation)) {
                        if (normalizeFiveSimCountryCode(activation?.serviceCode, DEFAULT_FIVE_SIM_PRODUCT) !== 'whatsapp') {
                          ...
                          replaceReason = 'whatsapp_requires_product_switch';
                          await addLog(
                            `步骤 9：验证码被拒后的重发入口切换为 WhatsApp 通道，但当前 5sim 订单产品为 ${activation?.serviceCode || DEFAULT_FIVE_SIM_PRODUCT}，将重新申请 WhatsApp 号码。`,
                            'warn'
                          );
                          break;
                        }
                        await addLog(
                          `步骤 9：验证码被拒后的重发入口切换为 WhatsApp 通道（${resendResult.channelText || resendResult.text || 'WhatsApp'}），当前 5sim 订单将继续沿用并轮询接码平台消息。`,
                          'warn'
                        );
                        await addLog(
