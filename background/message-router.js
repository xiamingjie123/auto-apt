  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(message, sender) {
      switch (message.type) {
        ...
        case 'DETECT_CLASH_VERGE_CONTROLLER': {
          if (typeof detectClashVergeController !== 'function') {
            throw new Error('Clash Verge 控制器探测能力尚未接入。');
          }
          const state = await getState();
          const result = await detectClashVergeController({
            ...(state || {}),
            ...(message.payload || {}),
          });
          return { ok: true, ...result };
